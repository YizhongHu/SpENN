"""Create compact study snapshots from the latest final report.
...
def build_sync_plan(
    *,
    ...
    """Return source files needed for one study snapshot."""
...
def _run_metadata_files_from_collection_roots(
    ...
    for root in roots:
        ...
        if not _is_stage_root(root, results_root, STAGE_COLLECT):
            continue
        sources = read_json_object_list(root / "source_validation_attempts.json", warnings)
        for source in sources:
            ...
            source_train = read_json_object(validation_dir / "source_train_attempt.json", warnings)
...
def _study_name(config: Any) -> str:
    return (
        _config_text(config, "study")
        or _config_text(config, "study.name")
        or _config_text(config, "experiment.name")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    _print_status(f"{log_prefix(_study_name(config))} planning {mode} under {args.destination}")
