...
```bash
uv run --locked --extra cpu python experiments/atomistic/he-cutover/cutover_plan.py --facility cannon --results-root "${TPEN_RESULTS_ROOT:?}" --plan-attempt-id "${TPEN_PLAN_ATTEMPT_ID:?}"
```
...
Polaris planning uses the same command with `--facility polaris` from the approved overlay, followed by inspection and:
