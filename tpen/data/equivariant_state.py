...
runtime validation is a separate contract in :mod:`tpen.data.validation`.
...
from tpen.data.permutation import Permutable, Permutation
...
def apply_particle_permutation(value: Any, permutation: Permutation) -> Any:
    ...
    ``.validate()`` / ``.validity_metrics()`` (see :mod:`tpen.data.validation`),
