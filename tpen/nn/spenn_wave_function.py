...
from tpen.nn.h2_nuclear_confinement import (
...
@dataclass(frozen=True)
class H2NuclearFactorizedWavefunctionParts:
    """Typed H2 factorization with the smooth tail kept in regular logabs.

    ``nuclear.value`` contains only the bounded pair cusp. The molecular tail
    is already included in ``regular_logabs`` and is therefore never exposed as
    a Coulomb-cancellation term.
    """

    regular_logabs: torch.Tensor
    nuclear: H2NuclearConfinementEvaluation
    sign: torch.Tensor
    phase: torch.Tensor | None = None
    aux: dict[str, Any] = field(default_factory=dict)

    def validate(self, batch: ElectronBatch) -> "H2NuclearFactorizedWavefunctionParts":
        """Validate H2 output and factorization shape semantics."""

        flat = batch.flatten_samples()
        expected = (flat.batch_size,)
        if self.regular_logabs.shape != expected or self.sign.shape != expected:
            raise ValueError(
                f"H2NuclearFactorizedWavefunctionParts scalar fields must have shape {expected}"
            )
        if self.phase is not None and self.phase.shape != expected:
            raise ValueError(f"H2NuclearFactorizedWavefunctionParts.phase must have shape {expected}")
        for name, value in (("regular_logabs", self.regular_logabs), ("sign", self.sign)):
            if value.device != flat.device or value.dtype != flat.dtype:
                raise ValueError(
                    f"H2NuclearFactorizedWavefunctionParts.{name} must match batch dtype/device"
                )
        if self.phase is not None and (
            self.phase.device != flat.device or self.phase.dtype != flat.dtype
        ):
            raise ValueError("H2NuclearFactorizedWavefunctionParts.phase must match batch dtype/device")
        self.nuclear.validate(flat)
        self.as_output().validate(batch_size=flat.batch_size)
        return self

    def permute(self, permutation: Permutation) -> "H2NuclearFactorizedWavefunctionParts":
        """Apply fermionic parity while permuting H2 pair fields by electron."""
    ...
    def permute_nuclei(self, permutation: Permutation) -> "H2NuclearFactorizedWavefunctionParts":
        """Relabel identical H nuclei without changing physical scalar fields."""

        return type(self)(
            regular_logabs=self.regular_logabs.clone(),
            nuclear=self.nuclear.permute_nuclei(permutation),
    ...
    def compare(
        self,
        other: "H2NuclearFactorizedWavefunctionParts",
        *,
        atol: float = 1.0e-6,
        rtol: float = 1.0e-6,
    ) -> tuple[bool, dict[str, float]]:
        """Compare physical scalars and every explicit H2 nuclear field."""
        ...
        nuclear_ok, nuclear_metrics = self.nuclear.compare(other.nuclear, atol=atol, rtol=rtol)
        return scalar_ok and nuclear_ok, {
            "max_abs_error": max(
                scalar_metrics["max_abs_error"], nuclear_metrics["max_abs_error"]
            )
        }
...
class TPENWaveFunction(EquivariantMap):
    ...
    nuclear_envelope : NuclearFactorizedEnvelope or None, optional
        Explicit single-centre atomic envelope construction.
    h2_nuclear_envelope : H2NuclearFactorizedEnvelope or None, optional
        Separate H2-only bounded-cusp and smooth-tail construction.
    ...
    **kwargs : object
    ...
    Notes
    -----
    Exactly one of ``envelope``, ``nuclear_envelope``, and
    ``h2_nuclear_envelope`` must be supplied.
    """
    ...
    def __init__(
        ...
        *,
        ...
        h2_nuclear_envelope: H2NuclearFactorizedEnvelope | None = None,
        basis: nn.Module | None = None,
        **kwargs,
    ) -> None:
        super().__init__(**kwargs)
        if sum(item is not None for item in (envelope, nuclear_envelope, h2_nuclear_envelope)) != 1:
            raise ValueError(
                "TPENWaveFunction requires exactly one of envelope, nuclear_envelope, "
                "or h2_nuclear_envelope"
            )
        self.basis = basis
        ...
        self.h2_nuclear_envelope = h2_nuclear_envelope
    ...
    def forward_impl(self, batch: ElectronBatch) -> WavefunctionOutput:
        ...
        if self.h2_nuclear_envelope is not None:
            output = self._readout_output(batch)
            regular_envelope, cusp = self.h2_nuclear_envelope.factorized_value(batch)
            return WavefunctionOutput(
                logabs=(output.logabs + regular_envelope) + cusp,
                sign=output.sign,
                phase=output.phase,
                aux=dict(output.aux),
            )
    ...
    def h2_nuclear_factorization(
        self, batch: ElectronBatch
    ) -> H2NuclearFactorizedWavefunctionParts:
        """Return the explicit H2 cusp/tail factorization."""

        if self.h2_nuclear_envelope is None:
            raise ValueError("TPENWaveFunction has no H2NuclearFactorizedEnvelope")
        output = self._readout_output(batch)
        evaluation = self.h2_nuclear_envelope.nuclear_confinement.evaluate(batch)
        regular = output.logabs + _log_factor(
            self.h2_nuclear_envelope.regular_envelope,
            batch,
            output.logabs.shape,
            name="regular_envelope",
        )
        regular = regular + evaluation.smooth_tail_logabs.sum(dim=1)
        return H2NuclearFactorizedWavefunctionParts(
            regular_logabs=regular,
            nuclear=evaluation,
            sign=output.sign,
            phase=output.phase,
