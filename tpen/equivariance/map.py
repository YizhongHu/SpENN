...
runtime checkers in :mod:`tpen.equivariance.checks` and by pytest-only helpers
under ``tests/``.
"""
...
from tpen.trace import trace_value
...
class EquivariantMap(nn.Module, ABC):
    ...
    :mod:`tpen.equivariance.checks` and pytest-only helpers under ``tests/``.
