...
class HeliumAtlasCalculator:
    ...
    def calculate(
        ...
        *,
        ...
        if not isinstance(local_result, LocalEnergyResult):
            raise TypeError(
                "HeliumAtlasCalculator requires a decomposed LocalEnergyResult"
            )
        ...
        if kinetic_names:
            if local_result.per_electron_kinetic is None:
                raise ValueError(
                    "the configured KineticEnergy term did not provide its typed "
                    "per-electron attribution"
                )
            per_electron = local_result.per_electron_kinetic.to(dtype=torch.float64)
        else:
            if local_result.per_electron_kinetic is not None:
                raise ValueError(
                    "per-electron kinetic attribution requires a configured "
                    "KineticEnergy registry term"
                )
...
def _realized_coordinate(
    ...
    electron_nuclear_radius = electron_nuclear_displacements(batch).norm(dim=-1)
    if electron_nuclear_radius.shape[2] != 1:
        raise ValueError("helium atlas coordinates require exactly one nucleus")
    ...
    for index, kind in enumerate(coordinate_kinds):
        if kind == "electron_electron_distance":
            value = geometry.electron_electron_distance[index]
            ...
            value = electron_nuclear_radius[index, electron, 0]
        else:
