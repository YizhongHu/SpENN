...
class FactorResponseCalculator:
    ...
    def __init__(self, *, hamiltonian_terms: Sequence[HamiltonianTerm] | Mapping[str, HamiltonianTerm],
                 arms: Sequence[Mapping[str, object] | FactorParameterScale], baseline_label: str = "baseline",
                 chunk_size: int | None = None) -> None:
        self.hamiltonian_terms = normalize_hamiltonian_terms(hamiltonian_terms)
        self.arms = tuple(arm if isinstance(arm, FactorParameterScale) else FactorParameterScale.from_mapping(arm)
                          for arm in arms)
    ...
    def calculate(self, *, model: torch.nn.Module, bundle: EvaluationBundle,
                  context: EvaluationContext) -> EvaluationBundle:
        ...
        for arm in self.arms:
            with helium_factor_parameter_scale(model, arm) as realized:
                result = evaluate_local_energy_in_chunks(self.hamiltonian_terms, model, batch,
                                                         return_terms=True, chunk_size=self.chunk_size)
            if not isinstance(result, LocalEnergyResult) or result.wavefunction_output is None:
                raise TypeError("factor response requires LocalEnergyResult with wavefunction primitives")
            values.append(FactorResponseArmValues(
                label=arm.label,
                parameter_scales={"b_ee": arm.b_ee, "c_electron_nucleus": arm.c_electron_nucleus,
                                  "d_electron_nucleus": arm.d_electron_nucleus},
                realized_parameters=dict(realized), local_energy=result.total.detach(),
                logabs=result.wavefunction_output.logabs.detach().reshape(-1),
                sign=result.wavefunction_output.sign.detach().reshape(-1),
                term_energies={name: term.detach().reshape(-1) for name, term in result.terms.items()},
            ))
        return replace(bundle, factor_response=FactorResponseValues(
            comparison_kind="common_configuration", baseline_label=self.baseline_label,
            arms=tuple(values), model_state_restored=True))
...
class FactorArmCalculator:
    ...
    def __init__(self, *, calculator: object, arm: Mapping[str, object] | FactorParameterScale) -> None:
    ...
    def calculate(self, *, model: torch.nn.Module, bundle: EvaluationBundle,
                  context: EvaluationContext) -> EvaluationBundle:
        """Delegate only when generated configurations identify the same arm."""

        metadata = bundle.generated.metadata
        if metadata.get("comparison_kind") != "re_equilibrated":
            raise ValueError("FactorArmCalculator requires re-equilibrated generated configurations")
        if metadata.get("factor_arm") != self.arm.label:
            raise ValueError("factor calculator arm does not match generated configurations: "
                             f"{self.arm.label!r} != {metadata.get('factor_arm')!r}")
        with helium_factor_parameter_scale(model, self.arm):
            return self.calculator.calculate(model=model, bundle=bundle, context=context)
