...
class FactorResponseSummary:
    ...
    def __init__(
        self,
        *,
        filename: str = "factor_response.csv",
    ...
    def summarize(
        ...
        *,
        ...
        """Persist every common-configuration row without truncation."""
        ...
        if required > self.max_records:
            raise ValueError(
                "FactorResponseSummary max_records would truncate the common grid: "
        ...
        metrics: dict[str, MetricScalar] = {
            "comparison_is_common_configuration": True,
            "arm_count": len(arms),
            "configuration_count": n_samples,
            ...
            for arm in arms:
                ...
                for index in range(n_samples):
                    row: dict[str, object] = {
                        ...
                        "sign": float(arm.sign.reshape(-1)[index].item()),
        ...
        return SummaryResult(
            ...
            artifacts=(
                ArtifactRecord(
                    ...
                    metadata={
                        "comparison_kind": response.comparison_kind,
                        "baseline_label": response.baseline_label,
                        "rows": required,
                        "arm_count": len(arms),
                        "configuration_count": n_samples,
...
def _arm_by_label(
    arms: tuple[FactorResponseArmValues, ...],
    label: str,
...
def _validate_arm(
    arm: FactorResponseArmValues,
    *,
    n_samples: int,
    term_names: tuple[str, ...] | None = None,
) -> None:
    for name, value in (
        ("local_energy", arm.local_energy),
        ("logabs", arm.logabs),
        ("sign", arm.sign),
    ):
...
def _arm_metrics(
    ...
    *,
    ...
    delta_logabs: torch.Tensor,
) -> Mapping[str, MetricScalar]:
    ...
    return {
        f"{prefix}/local_energy_mean": float(finite_energy.mean().item()),
