...
class FactorParameterScale:
    ...
    def from_mapping(cls, value: Mapping[str, object]) -> "FactorParameterScale":
        """Build one arm while rejecting unknown or incomplete identities."""
...
def helium_factor_parameter_scale(
    ...
    """Apply one He factor arm temporarily and restore every model parameter.

    The model contract is explicit: exactly one ``ElectronElectronCusp`` and
    exactly one ``ElectronNucleusCusp`` with a trainable
    ``CurvatureElectronNucleusCuspLaw`` must be present in ``model.factors``.
    No container is recursively inspected.
    """
    ...
    realized = {
        "b_ee": base["b_ee"] * resolved.b_ee,
        "c_electron_nucleus": base["c_electron_nucleus"] * resolved.c_electron_nucleus,
        "d_electron_nucleus": base["d_electron_nucleus"] * resolved.d_electron_nucleus,
    }
    try:
        with torch.no_grad():
            if resolved.b_ee != 1.0:
                ee_factor.raw_opposite_range.copy_(
                    _inverse_positive_parameter(
                        realized["b_ee"],
                        eps=float(ee_factor.eps),
                        like=ee_factor.raw_opposite_range,
                    )
                )
            if resolved.c_electron_nucleus != 1.0:
                en_law.raw_curvature_coefficient.copy_(
                    torch.as_tensor(
                        realized["c_electron_nucleus"],
                        device=en_law.raw_curvature_coefficient.device,
                        dtype=en_law.raw_curvature_coefficient.dtype,
                    )
                )
            if resolved.d_electron_nucleus != 1.0:
                en_law.raw_curvature_range.copy_(
                    _inverse_positive_parameter(
                        realized["d_electron_nucleus"],
                        eps=float(en_law.eps),
                        like=en_law.raw_curvature_range,
                    )
                )
        yield realized
    finally:
...
def _helium_factor_owners(
    model: torch.nn.Module,
) -> tuple[ElectronElectronCusp, CurvatureElectronNucleusCuspLaw]:
    ...
    if len(ee_factors) != 1 or len(en_factors) != 1:
        raise ValueError(
            "factor response requires exactly one ElectronElectronCusp and one "
    ...
    if not isinstance(en_law, CurvatureElectronNucleusCuspLaw) or not en_law.trainable:
        raise ValueError(
            "factor response requires a trainable CurvatureElectronNucleusCuspLaw"
        )
    return ee_factor, en_law


def _inverse_positive_parameter(
    value: float,
    *,
    eps: float,
    like: torch.Tensor,
    ...
    # Stable inverse-softplus: x + log(-expm1(-x)).
