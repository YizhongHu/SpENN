...
This module is written by the DURABLE REVIEWER's test-implementation lane, not
by the implementation lane under review. It adds no production code and never
touches `claude/vmc-sr/lane-n` or `claude/vmc-sr/hydra-smokes`; it lives on its
own branch, `claude/vmc-sr/review-n-r1`, at the PR 476 (N3) tip.

Five probes, T1-T5:

* T1 (expected PASS) -- an asymmetric reducer proves score/energy centering is
  built from the reducer's REDUCED total, not from a local `.mean()` that only
  borrows the reducer's count.
* T2 (expected PASS) -- the QGT singular guard is bidirectional: it refuses a
  matrix that is truly singular at the tolerance boundary, and it does NOT
  refuse a well-conditioned matrix whose smallest eigenvalue merely happens to
  be small but legitimate.
* T3 (expected PASS; a FIRST-CLASS FINDING if it fails) -- probes Lane N's own
  receipt claim that structurally inactive parameters will emit exactly-zero
  score columns once the score seam is fixed, and that this needs no change on
  the SR side, at the engine level (geometry + QGT + both solve routes).
* T4 (expected PASS) -- a trainer checkpoint state carrying a stateful SR
  method's envelope is `json.dumps`-serializable and round-trips exactly.
* T5 (expected FAIL -- RED DEMONSTRATION of reviewer finding F4) -- resuming a
  stateful method from checkpoint state that is missing the `update_method`
  key silently succeeds today, leaving the method's own counters diverged from
  the trainer's. This test encodes the DESIRED behavior (a loud raise) and is
  therefore expected to fail against current code. It is run separately on
  Cannon so its failure does not pollute the T1-T4 pass/fail accounting.

Tolerances mirror the sibling contract-test modules this file draws helpers
and conventions from (`tests/unit/training/test_score_geometry.py`,
`tests/unit/training/test_qgt.py`, `tests/unit/training/test_sr_trainer_integration.py`):
float64 problems, `1e-9`-`1e-12` for well-conditioned direct comparisons, with
an explicitly looser, explicitly justified tolerance in T2(a) where the
construction is deliberately near the edge of float64's numerical rank.
"""
