...
def test_model_only_restore_loads_weights_into_configured_model(tmp_path: Path) -> None:
    ...
    report = restore_checkpoint(
        checkpoint={"path": str(root), "restore_mode": "model_only", "strict": True},
        model=fresh,
        context=_context(),
    )
...
def test_restore_rejects_checkpoint_without_complete_marker(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="COMPLETE"):
        restore_checkpoint(
            checkpoint={"path": str(checkpoint_dir), "restore_mode": "model_only"},
...
def test_train_resume_restores_all_train_state(tmp_path: Path) -> None:
    ...
    report = restore_checkpoint(
        checkpoint={"path": str(checkpoint_dir), "restore_mode": "train_resume"},
        model=fresh,
        optimizer=optimizer,
        trainer=trainer,
        sampler=sampler,
...
def test_restore_strict_load_fails_on_unexpected_keys(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="ghost"):
        restore_checkpoint(
            checkpoint={"path": str(checkpoint_dir), "restore_mode": "model_only", "strict": True},
            model=torch.nn.Linear(3, 2).double(),
            context=_context(),
        )
