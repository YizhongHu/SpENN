...
from tpen.evaluation.factor_response import (
...
def _physical(model: _HeliumFactorModel) -> tuple[float, float, float]:
    ...
    return (
        float(ee.opposite_range_parameter.detach().item()),
        float(en.curvature_coefficient.detach().item()),
        float(en.curvature_range.detach().item()),
    )
...
def _bundle(*, metadata: dict | None = None) -> EvaluationBundle:
    ...
    return EvaluationBundle(
        generated=GeneratedConfigurations(batch=batch, metadata=metadata or {})
    )
...
def test_factor_scale_changes_physical_values_and_restores_every_parameter() -> None:
    ...
    arm = FactorParameterScale(
        label="response",
        b_ee=0.9,
        c_electron_nucleus=1.1,
        d_electron_nucleus=0.9,
    )
    with helium_factor_parameter_scale(model, arm) as realized:
        inside = _physical(model)
        assert inside == pytest.approx((before[0] * 0.9, before[1] * 1.1, before[2] * 0.9))
        assert tuple(realized.values()) == pytest.approx(inside)
    assert _physical(model) == pytest.approx(before)
    assert all(
        torch.equal(value, before_state[name])
...
def test_factor_scope_restores_after_delegate_failure() -> None:
    ...
    with pytest.raises(RuntimeError, match="synthetic"):
        with helium_factor_parameter_scale(
            model,
            {"label": "bad", "b_ee": 1.1},
        ):
            raise RuntimeError("synthetic")
    assert all(
        torch.equal(value, before[name]) for name, value in model.named_parameters()
    )


def test_factor_arm_calculator_reapplies_matching_reequilibrated_arm(
    tmp_path: Path,
) -> None:
    model = _HeliumFactorModel()
    before = _physical(model)
    ...
    calculator = FactorArmCalculator(
        calculator=_Delegate(),
        arm={"label": "b_plus", "b_ee": 1.1},
    )
    bundle = _bundle(
        metadata={"comparison_kind": "re_equilibrated", "factor_arm": "b_plus"}
    )
...
def test_common_configuration_summary_writes_complete_aligned_grid(
    tmp_path: Path,
) -> None:
    bundle = _bundle()
    arms = tuple(
        FactorResponseArmValues(
            label=label,
            parameter_scales={"b_ee": scale},
            realized_parameters={"b_ee": scale},
            local_energy=torch.tensor(values, dtype=torch.float64),
            logabs=torch.tensor([-1.0, -2.0], dtype=torch.float64) * scale,
            sign=torch.ones(2, dtype=torch.float64),
            term_energies={
                "kinetic": torch.tensor(values, dtype=torch.float64),
            },
        )
    ...
    result = FactorResponseSummary(max_records=4).summarize(
        bundle=bundle,
        context=_context(tmp_path),
        namespace="test",
    )
    ...
    with pytest.raises(ValueError, match="truncate"):
        FactorResponseSummary(filename="too_small.csv", max_records=3).summarize(
            bundle=bundle,
            context=_context(tmp_path),
            namespace="test",
        )
