...
class ElectronBatch:
    ...
    spins : torch.Tensor or None, optional
        Optional spin labels with shape ``[*sample_shape, n_electrons]``.
    ...
    def __post_init__(self) -> None:
        ...
        if self.spins is not None and tuple(self.spins.shape) != self.sample_shape + (self.n_electrons,):
...
class Walkers:
    ...
    spins : torch.Tensor or None, optional
        Optional spin labels aligned with `positions`.
    ...
    def __post_init__(self) -> None:
        ...
        if self.spins is not None and self.spins.shape[:2] != self.positions.shape[:2]:
            raise ValueError("Walkers.spins must align with positions")
...
def validate_batch(batch: ElectronBatch) -> None:
    """Validate an electron batch against order-one tensor conventions.
    ...
    ------
    ValueError
        If the position tensor does not satisfy the expected shape
        conventions.
    """

    flat_batch = batch.flatten_samples()
    validate_irrep_tensor(flat_batch.positions, order=1, batch_size=flat_batch.batch_size, n_electrons=flat_batch.n_electrons)
